  ...
  Type objective_function <Type>::operator() () {
    ...
    */
    ...
    for (int year = 0; year < (numberofyears - 1); year++) {
      ...
      matrix<Type> suplusdist = (rho * currentsharedusers) + ((1-rho) * azzalinimat);
      ...
      matrix<Type> FOIforayear = weightedmatrix * incidenceofyear;
      ...
      for (int county = 0; county < dim; county++) {
        if (fullcountyincidence(county,year) == 0) {
          ...
          Type linearpred = logFOIforayear + exp(logsd_Year)*YearRandomEffect(year) +
            exp(logsd_County)*CountyRandomEffect(county);
