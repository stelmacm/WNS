...
px[0] = exp( logspace_add(tx[0], tx[0]-ty[0]) ) * py[0];
  ...
  Type objective_function <Type>::operator() () {
    ...
    DATA_INTEGER(dim);
    DATA_MATRIX(SM);   // shared-user matrix (county i, county j)
    // FIXME: consider passing as a {ncounty, ncounty, year} array for
    //  indexing convenience below ...
    ...
    weightedmatrix.setZero(); // ?? why do we need this ??
    for(int i = 0; i < dim; i++){
      vector<Type> currentrow = azzalinimat.row(i);
      Type sumofvec = currentrow.sum(); // azzalinimat.row(i).sum() ??
      vector<Type> newrow = currentrow / sumofvec;
      // Reattach the weighted vector to the weightedmatrix
