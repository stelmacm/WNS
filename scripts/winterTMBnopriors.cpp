...
px[0] = exp( logspace_add(tx[0], tx[0]-ty[0]) ) * py[0];
  ...
  Type objective_function <Type>::operator() () {
    ...
    //DATA_SCALAR(logsdyear_mean);
    //DATA_SCALAR(logsdyear_sd);
    //DATA_SCALAR(logsdcounty_mean);
    //DATA_SCALAR(logsdcounty_sd);
    ...
    PARAMETER(logtheta);  // reasonable range for this is 0.5-3
    ...
    //Here I wil row normalize the azzalini matrix
    vector<Type> matrixrowmean(dim);
    for(int i = 0; i < dim; i++){
      vector<Type> currow = (azzalinimat.row(i));
      matrixrowmean(i) = currow.mean();
    }
    Type mrm = matrixrowmean.mean();
    
    matrix<Type> rownormazzalinimat = azzalinimat / mrm;
    
    //Create row sum 1 weight matrix of azzalini distances
    //Realizing this might be unneccessary
    ...
    /*
     SU' = SU/mean(rowMeans(SU))
     D' = D/mean(rowMeans(D))
     M = rho*SU' + (1-rho)*D'
     */
    //So we need to take the mean of the rowMeans of the computed azzelini matrix
    //azzalinimeanmat = azzalinimat* (1/mean(rowMeans(azzalinimat))); //Cant divide matrix types
    //SS/(SS.sum()/proddims)
    //Shared users will already come averaged in
    ...
    for (int year = 0; year < (numberofyears - 1); year++) {
      ...
      matrix<Type> suplusdist = (rho * currentsharedusers) + ((1-rho) * rownormazzalinimat);
      //Now I want to row normalize this matrix so that the same *overall* weight?
      //Seems vague to me
      /*
       matrix<Type> weightedmatrix(dim, dim);
       weightedmatrix.setZero();
       for(int i = 0; i < dim; i++){
       vector<Type> currentrow = suplusdist.row(i);
       Type sumofvec = currentrow.sum();
       vector<Type> newrow = currentrow / sumofvec;
       //Reattach the weighted vector to the weightedmatrx
       weightedmatrix.row(i) = newrow;
       }*/
      //Now we extract the correct winter year
      matrix<Type> currentwinteryear = numberofwinterdays.block(0, year, dim, 1);
      
      //Extracting incidence for a given year 
      ...
      for (int county = 0; county < dim; county++) {
        if (fullcountyincidence(county,year) == 0) {
          ...
          Type countywinterdays = currentwinteryear(county, 0);
          
          Type individFOIforayear = FOIforayear(county,0);
          Type logFOIforayear = log(individFOIforayear + offsetparam);
          Type linearpred = logFOIforayear + countywinterdays + YearRandomEffect(year) + CountyRandomEffect(county);
    ...
    //1:  nll -= .... + b[i]; nll -= sum(dnorm(b, 0, exp(logsd)))
    //2: eta[i] = ... + exp(logsd)*u[i]; nll -= sum(dnorm(u, 0, 1)) u is standardized version of b
    ...
    //nll -= dnorm(logsd_Year, logsdyear_mean, logsdyear_sd, true);
    //nll -= dnorm(logsd_County, logsdcounty_mean, logsdcounty_sd, true);
